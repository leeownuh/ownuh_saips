...
$bypassToken = trim((string)($body['bypass_token'] ?? ''));

// bypass_token path skips factor/code requirement
if (!$mfaToken || (!$bypassToken && (!$factor || !$code))) {
    http_response_code(400);
    echo json_encode(['status' => 'error', 'code' => 'VALIDATION_ERROR', 'message' => 'mfa_token and either (factor + code) or bypass_token are required.']);
    exit;
}
...
// ── MFA Bypass Token path (SRS §2.4.1) ───────────────────────────────────────
if ($bypassToken) {
    $bypassHash = hash('sha256', $bypassToken);

    // Fast path: check Redis first
    $bypassJson = $redis->get("saips:mfa_bypass:{$bypassHash}");
    if ($bypassJson) {
        $bypassData = json_decode($bypassJson, true);
        if (($bypassData['user_id'] ?? null) === $userId) {
            // Consume token immediately (single-use)
            $redis->del("saips:mfa_bypass:{$bypassHash}");
            $pdo->prepare('UPDATE users SET mfa_bypass_token = NULL, mfa_bypass_expiry = NULL WHERE id = ?')
                ->execute([$userId]);
            AuditMiddleware::log('AUTH-MFA', 'MFA Bypass Token Used', $userId, null, null, null, null, null, [
                'admin_id' => $bypassData['admin_id'] ?? null,
                'reason'   => $bypassData['reason']   ?? null,
            ]);
            $verified = true;
        }
    }
    ...
    if (!$verified) {
        // Slow path: check DB (handles Redis miss / server restart)
        $stmt = $pdo->prepare(
            'SELECT mfa_bypass_token, mfa_bypass_expiry FROM users WHERE id = ? AND mfa_bypass_token IS NOT NULL'
        );
        $stmt->execute([$userId]);
        $row = $stmt->fetch();

        if ($row
            && hash_equals($row['mfa_bypass_token'], $bypassHash)
            && strtotime($row['mfa_bypass_expiry']) > time()
        ) {
            // Consume token (single-use)
            $pdo->prepare('UPDATE users SET mfa_bypass_token = NULL, mfa_bypass_expiry = NULL WHERE id = ?')
                ->execute([$userId]);
            AuditMiddleware::log('AUTH-MFA', 'MFA Bypass Token Used (DB path)', $userId, null, null, null, null, null, []);
            $verified = true;
        }
    }

    if (!$verified) {
        AuditMiddleware::authFailure($userId, $pending['ip'], 'mfa_bypass_invalid', 0);
        http_response_code(401);
        echo json_encode(['status' => 'error', 'code' => 'UNAUTHORIZED', 'message' => 'Bypass token is invalid or has expired.']);
        exit;
    }

    // Skip the factor switch — fall through to token issuance below
    $factor = 'bypass';
}

if (!$verified) switch ($factor) {
    case 'totp':
