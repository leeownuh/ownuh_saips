...
if (($_SERVER['REQUEST_METHOD'] ?? 'GET') !== 'POST') {
    ...
    echo json_encode(['status' => 'error', 'code' => 'METHOD_NOT_ALLOWED', 'message' => 'Use POST for attack attribution analysis.']);
    exit;
}
...
$result = (new MLService())->analyzeAttackAttribution(
    $input['date_from'] ?? null,
    $input['date_to'] ?? null,
    (int)($input['limit'] ?? 1200),
    filter_var($input['with_llm'] ?? false, FILTER_VALIDATE_BOOLEAN)
);
